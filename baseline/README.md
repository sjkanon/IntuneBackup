...
```mermaid
flowchart LR
  T["IntuneTemplate/"] -->|generate-baseline.js| B["baseline/intune/baseline-v1.0.json<br/>158 rules"]
...
158 regels: 6 die uit het platform zelf komen (checkId 001–006, device-compliance- en
app-protection-checks) plus 152 gegenereerd uit `IntuneTemplate/`. Daarvan 18 met severity
`high`, de rest `medium`.

| `type` | Aantal | Uit |
|---|---:|---|
| `settings-catalog-match` | 151 | elke Settings Catalog-policy, Windows én macOS |
...
Per platform: Windows 111, macOS 30, iOS/iPadOS 8, Android 3.
