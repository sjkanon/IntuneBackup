...
```mermaid
flowchart LR
  T["IntuneTemplate/"] -->|generate-baseline.js| B["baseline/intune/baseline-v1.0.json<br/>160 rules"]
...
160 regels: 6 die uit het platform zelf komen (checkId 001–006, device-compliance- en
app-protection-checks) plus 154 gegenereerd uit `IntuneTemplate/`. Daarvan 18 met severity
`high`, de rest `medium`.

| `type` | Aantal | Uit |
|---|---:|---|
| `settings-catalog-match` | 153 | elke Settings Catalog-policy, Windows én macOS |
...
Per platform: Windows 113, macOS 30, iOS/iPadOS 8, Android 3.
